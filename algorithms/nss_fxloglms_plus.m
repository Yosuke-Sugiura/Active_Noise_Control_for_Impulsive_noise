function [w, params] = nss_fxloglms_plus(w, e, r_buf, ~, mu, params)
...
%   E_e    : Smoothed squared error energy (updated internally)
%   lmd_e  : Forgetting factor for E_e update

    sgn = sign(e);
    abs_e = params.G_ml * abs(e) + 1;
    e_c = sgn * log(abs_e) / abs_e;
    params.E_e = params.lmd_e * params.E_e + (1 - params.lmd_e) * abs(e)^2;
    mu_ = mu / (sum(r_buf.^2) + params.E_e + 1e-3);
    w = w - mu_ * e_c * r_buf;
end
